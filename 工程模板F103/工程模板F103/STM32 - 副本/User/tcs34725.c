...
void TCS34725_I2C_Init()
{
    ...
    RCC_APB2PeriphClockCmd(TCS_SDA_CLOCK, ENABLE);
    RCC_APB2PeriphClockCmd(TCS_SCL_CLOCK, ENABLE);

    GPIO_InitStructure.GPIO_Pin = TCS_SCL_PIN;      // PB10/PB10=外接I2C
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;//通用推挽输出	
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度
    GPIO_Init(TCS_SCL_X, &GPIO_InitStructure);       // 对选中管脚初始化
    GPIO_InitStructure.GPIO_Pin = TCS_SDA_PIN;       // PB10/PB10=外接I2C
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;//通用推挽输出	
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//速度
    GPIO_Init(TCS_SDA_X, &GPIO_InitStructure);       // 对选中管脚初始化

    GPIO_SetBits(TCS_SCL_X, TCS_SCL_PIN); // 高电平
    GPIO_SetBits(TCS_SDA_X, TCS_SDA_PIN); // 高电平
}
void TCS_SDA_In(void)
{
    ...
    GPIO_Init(TCS_SDA_X, &GPIO_InitStructure);
}
...
u8 TCS34725_Init(void)
{
	...
	TCS34725_I2C_Init(); 
	TCS34725_Read(0x12, &id, 1);  //TCS34725 的 ID 是 0x44 可以根据这个来判断是否成功连接,0x4D是TCS34727;
	if(id==0x4D | id==0x44)
		{
